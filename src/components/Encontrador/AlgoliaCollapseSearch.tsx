...
const RefinementListCustom = ({ items, refine, createURL }) => (
  <ul>
    {items.map((item) => (
      <li key={item.label}>
        <label
          ...
          } ${item.label === "true" ? "inStock " : "outStock"} `}
        ...
        >
          ...
          >
            {item.label === "true" ? <>En Stock</> : <>Por encargo</>}
          </span>
          <span className="font-light ais-RefinementList-count">
            {item.count}
          </span>
        </label>
      </li>
...
const EncontrarComponent = () => {
  return (
    <>
      ...
      <Disclosure>
        <div className="flex justify-between w-full px-4 py-0 mt-3 text-sm font-medium text-left text-gray-900 duration-700 bg-gray-100 rounded-lg hover:bg-gray-100 focus:outline-none focus-visible:ring focus-visible:ring-green-500 focus:ring-green-500 focus-visible:ring-opacity-75">
        ...
        <div className="px-1 pt-1 text-base text-gray-700 bg-white shadow-md">
          <CustomRefinementList attribute="GameInStock" />
        </div>
      </Disclosure>
